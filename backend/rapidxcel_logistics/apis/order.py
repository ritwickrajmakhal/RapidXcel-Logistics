...
def create_order():
    ...
    required_fields = ['customer_id', 'shipping_address', 'pin_code',
                       'phone_number', 'items', 'location_type', 'courier_service_id']
    ...
    try:
        new_order = Order(
            ...
            consignment_weight=sum(item['weight'] for item in data['items']),
            shipping_cost=calculate_shipping_cost(
                sum(item['weight'] for item in data['items']), data['location_type']),
        ...
        for item in data['items']:
            required_fields = ['stock_id', 'product_name',
    ...
    try:
        ...
        if status_updated:
            ...
            notification_message = status_messages.get(
                order.status, f'Your order status has been updated to {order.status}.')
